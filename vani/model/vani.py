...
Full Vani model: assembles all components into a unified system.
...
from vani.tokenizer.rvq_tokenizer import VaniRVQTokenizer, RVQConfig
from vani.tokenizer.script_adapter import ScriptFamilyAdapter, ScriptAdapterConfig
from vani.model.slow_ar import SlowAR, SlowARConfig
from vani.model.fast_ar import FastAR, FastARConfig
...
class VaniModel(nn.Module):
    """
    Vani: Voice of India — full foundational TTS model.

    This class is the entry point for both training and inference.
    For serving, use vani.inference.engine.VaniEngine (SGLang-wrapped).
    """
    def __init__(self, cfg: Optional[VaniConfig] = None):
        super().__init__()
        self.cfg = cfg or VaniConfig()

        self.tokenizer  = VaniRVQTokenizer(self.cfg.rvq)
    ...
    def synthesize(
        ...
        """
        from vani.tokenizer.text_processor import TextProcessor
    ...
    def from_pretrained(
        ...
        """
        Load a pretrained Vani model from HuggingFace Hub or local path.
        model_id: e.g. "india-ai/vani-sovereign-live"
        """
        import json
