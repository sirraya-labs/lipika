"""
lipika/inference/engine.py

Production inference engine for Lipika.
...
This file wraps the Lipika model for deployment in three configurations:
  1. Lipika-Sovereign-Pro:  Full quality, batch offline synthesis
  2. Lipika-Sovereign-Live: Real-time streaming, < 100ms TTFA
  3. Lipika-Sovereign-Lite: On-device, quantized (INT4/INT8)
...
logger = logging.getLogger("lipika.engine")
...
class LipikaEngine:
    """
    Production serving engine for Lipika.

    Example usage:
        engine = LipikaEngine.from_pretrained("india-ai/lipika-sovereign-live")

        # Single request
        audio = await engine.synthesize_async("नमस्ते दुनिया", language="hi")

        # Streaming
        async for chunk in engine.stream_async("Hello from Lipika", language="en-IN"):
            play_audio(chunk)
    ...
    def from_pretrained(
        ...
        **engine_kwargs,
        ...
        from lipika.model.lipika import LipikaModel
        model = LipikaModel.from_pretrained(model_id, variant=variant, device=device)
        cfg = EngineConfig(**engine_kwargs)
        return cls(model, cfg)


def build_fastapi_app(engine: LipikaEngine):
    """
    Build a production FastAPI app serving Lipika over HTTP/WebSocket.
    ...
    app = FastAPI(title="Lipika TTS API", version="1.0.0")

    @app.get("/v1/health")
    async def health():
        return {"status": "ok", "model": "lipika-sovereign"}
